...
/* ==== 0.13.7 — indice de défilement de la modale (~20 % du contenu peut se cacher sous le
   pli sans qu'aucun signe ne l'indique — retour terrain 15/07). Deux gestes, presque
   invisibles : 1) classe fc-can-scroll → fondu bas (CSS) tant qu'il reste du contenu
   dessous ; 2) au premier affichage, une « respiration » (18px aller-retour, douce) montre
   le geste au lieu de l'expliquer. Respecte prefers-reduced-motion. Append-only. ==== */
(function () {
	...
	ready(function () {
		...
		var done = false;
		function cue() {
			var more = banner.scrollHeight - banner.clientHeight - banner.scrollTop > 6;
			banner.classList.toggle('fc-can-scroll', more);
		}
		function nudge() {
			cue();
			if (done) { return; }
			done = true;
			if (!banner.classList.contains('fc-can-scroll')) { return; }
			if (window.matchMedia && matchMedia('(prefers-reduced-motion: reduce)').matches) { return; }
			setTimeout(function () {
				try { banner.scrollTo({ top: 18, behavior: 'smooth' }); } catch (e) { banner.scrollTop = 18; }
				setTimeout(function () {
					try { banner.scrollTo({ top: 0, behavior: 'smooth' }); } catch (e) { banner.scrollTop = 0; }
				}, 520);
			}, 700);
		}
		banner.addEventListener('scroll', cue, { passive: true });
