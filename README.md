...
- **Services exemptés.** Depuis la 0.16.1, l'éditeur peut lister les services connus que son site charge SANS consentement (un lecteur YouTube sur un annuaire de chaînes, par exemple) : ils gardent leur `src`, aucune façade ne les recouvre, et le bandeau les affiche « Toujours actif ». Rien n'est exempté par défaut ; filtre `freecookie_exempt_services`.
- **Jour et nuit.** Depuis la 0.16.0, la carte suit le réglage clair/sombre du navigateur du visiteur (`prefers-color-scheme`), ou un mode forcé depuis les options ; palette monochrome par défaut (encre sur papier), deux couleurs sombres réglables.
